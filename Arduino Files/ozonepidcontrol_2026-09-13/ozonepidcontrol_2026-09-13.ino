...
float kp = .075; // For the full-size chamber 5V reached at least 500-690. ~690/5/2 //.010;// V output stabilized at ~190ppb 190/5 = 38. This is what the tutorials say, but shouldn't it be 5/190? //.032;
...
float Pcomponent = 0;
float Icomponent = 1.5; // tweak the initial I term to help things stabilize sooner.
float Dcomponent = 0;
float vout_in_volts;
...
/*Door Sensor Code notes,
The door could be NO or NC switch. 
We need to change the DOOR_SENSOR_NORMALLY_CLOSED based on the sensor.
The door_sensor_pin i would assume is digital and we need to write that based on how we wire the arduino
*/
const int door_sensor_pin ;
bool DOOR_SENSOR_NORMALLY_CLOSED = true;

int DOOR_OPEN = 0;
int OZONE_ON_BEFORE_DOOR = 0;
bool DOOR_INTERLOCK_TRIPPED = false;

int door_last_raw_state = HIGH;
unsigned long door_last_change_time = 0;
const unsigned long DOOR_DEBOUNCE_MS = 50 ;// we could change this if we face more issues when debugging.
...
void setup() {
  ...
  //door code
  door_last_raw_state = digitalRead(door_sensor_pin);
  DOOR_OPEN = rawStateMeansDoorOpen(door_last_raw_state) ? 1: 0;
}

void loop() {
check_door(); //door code
control_loop();
...
// more door code
bool rawStateMeansDoorOpen( int rawState) {
  if (DOOR_SENSOR_NORMALLY_CLOSED) {
...
void check_door(){
  ...
  if (raw != door_last_raw_state) {
    door_last_change_time = millis();
    door_last_raw_state = raw;
  }

  if ((millis()- door_last_change_time) > DOOR_DEBOUNCE_MS) {
    bool doorOpenNow = rawStateMeansDoorOpen(raw);

    if (doorOpenNow && DOOR_OPEN == 0) {
      DOOR_OPEN = 1;
      OZONE_ON_BEFORE_DOOR =  OZONE_ON;
      OZONE_ON = 0;
      DOOR_INTERLOCK_TRIPPED = true;

      digitalWrite(ballast_power_relay_pin, LOW);
      dac.setDACOutVoltage(0,0);
      DFRout = 0;
      DACout = 0;

      Serial.println("DOOR_OPEN:1 - safety interlock tripped");
    }
    else if (!doorOpenNow && DOOR_OPEN == 1) {
      DOOR_OPEN = 0;
      DOOR_INTERLOCK_TRIPPED = false;
      OZONE_ON = OZONE_ON_BEFORE_DOOR;

      Serial.println("DOOR_OPEN:0 - interlock cleared ");
      Serial.println(OZONE_ON);
...
void control_loop(){
  ...
  if(FIRST_CYCLE & elapsed_time >= CYCLETIME){
    ...
    process_value_alternate = process_value_alternate/1023*OZONEGAIN;
  ...
  if(!FIRST_CYCLE & elapsed_time >= CYCLETIME){
    ...
    if(DFRout > 10000){DFRout = 10000;} // Prevent sending a value greater than 255 (the maximum value) to the DAC.
